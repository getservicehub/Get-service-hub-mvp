...
function mapRow(row: ProfessionalRow, specialtyIds: string[]): MaybeDemoProfessional {
  return {
    id: row.id, slug: row.slug, displayName: row.display_name, profession: row.profession,
    specialtyIds, city: row.city, state: row.state, languages: row.languages, bio: row.bio,
    avatarUrl: row.avatar_url ?? undefined,
    yearsExperience: row.years_experience_value != null && row.years_experience_source
      ? { value: row.years_experience_value, source: row.years_experience_source } : undefined,
    ...
    joinedAt: row.joined_at, isDemo: row.is_demo,
  };
}

async function getSpecialtyIdsByProfessional(professionalIds: string[]): Promise<Record<string, string[]>> {
  if (professionalIds.length === 0) return {};
  const supabase = createPublicClient();
  const { data, error } = await supabase.from("pro_professional_specialties").select("professional_id, specialty_id").in("professional_id", professionalIds);
