    ...
    <header className="relative border-b border-[var(--pro-line)] bg-[var(--pro-bg)] z-10">
      <nav className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 md:px-8">
        ...
        <div className="hidden items-center gap-6 md:flex">
          {navLinks.map((link) => (
            <Link key={link.href} href={link.href} className="text-sm text-[var(--pro-text-muted)] transition-colors hover:text-[var(--pro-text)]">{link.label}</Link>
          ))}
          <LangToggle lang={lang} setLang={setLang} />
          <Link href="/pro/login" className="text-sm text-[var(--pro-text-muted)] hover:text-[var(--pro-text)]">{t.nav.login}</Link>
          <Link href="/pro/join" className="rounded-md bg-[var(--pro-gold)] px-4 py-2 text-sm font-semibold text-[var(--pro-navy)] transition-transform hover:scale-[1.03]">{t.nav.joinCta}</Link>
        </div>
        <button type="button" aria-label={menuOpen ? "Close menu" : "Open menu"} aria-expanded={menuOpen} onClick={() => setMenuOpen((v) => !v)} className="flex h-9 w-9 items-center justify-center text-[var(--pro-text)] md:hidden">
      ...
      {menuOpen && (
        <div className="border-t border-[var(--pro-line)] bg-[var(--pro-bg)] px-5 py-5 md:hidden">
          <div className="flex flex-col gap-4">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} onClick={() => setMenuOpen(false)} className="text-base text-[var(--pro-text)]">{link.label}</Link>
            ))}
            ...
            <Link href="/pro/join" onClick={() => setMenuOpen(false)} className="rounded-md bg-[var(--pro-gold)] px-4 py-3 text-center text-sm font-semibold text-[var(--pro-navy)]">{t.nav.joinCta}</Link>
          </div>
        </div>
      )}
    </header>
...
function LangToggle({ lang, setLang }: { lang: "es" | "en"; setLang: (l: "es" | "en") => void }) {
  return (
    <div className="flex overflow-hidden rounded border border-[var(--pro-line)] font-mono text-xs" role="group" aria-label="Language">
