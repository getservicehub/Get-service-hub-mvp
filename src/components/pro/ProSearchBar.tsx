  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-2 rounded-xl border border-[var(--pro-line)] bg-[var(--pro-panel)] p-2 sm:flex-row sm:items-center">
      <div className="flex flex-1 items-center gap-2 px-3">
        ...
        <input id="pro-search-query" type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t.hero.searchSpecialtyPlaceholder}
      ...
      <div className="flex flex-1 items-center gap-2 px-3">
        ...
        <input id="pro-search-location" type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder={t.hero.searchLocationPlaceholder}
          className="w-full bg-transparent py-3 text-sm text-[var(--pro-text)] placeholder:text-[var(--pro-text-muted)]/70 focus:outline-none" />
      </div>
      <button type="submit" className="rounded-lg bg-gradient-to-r from-[var(--pro-gold-bright)] to-[var(--pro-gold)] px-6 py-3 text-sm font-semibold text-[var(--pro-navy)] transition-transform hover:scale-[1.02]">{t.hero.searchButton}</button>
    </form>
  );
}
